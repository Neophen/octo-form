<template>
  <div
    ref="wrapper"
    :class="
      `octo-croppa-container ${img ? 'octo-croppa--has-target' : ''} ${
        passive ? 'octo-croppa--passive' : ''
      } ${disabled ? 'octo-croppa--disabled' : ''} ${
        disableClickToChoose ? 'octo-croppa--disabled-cc' : ''
      } ${
        disableDragToMove && disableScrollToZoom
          ? 'octo-croppa--disabled-mz'
          : ''
      } ${fileDraggedOver ? 'octo-croppa--dropzone' : ''}`
    "
...
export default {
  ...
  watch: {
    ...
    "imgData.startX": function() {
      if (this.hasImage()) {
        this.$nextTick(this._draw);
      }
    },
    "imgData.startY": function() {
      if (this.hasImage()) {
        this.$nextTick(this._draw);
      }
    },
  ...
  methods: {
    ...
    addClipPlugin(plugin) {
      ...
      if (
        ...
        this.clipPlugins.indexOf(plugin) < 0
      ) {
    ...
    _setContainerSize() {
      if (this.useAutoSizing) {
        this.realWidth = +getComputedStyle(this.$refs.wrapper).width.slice(
          0,
          -2
        );
        this.realHeight = +getComputedStyle(this.$refs.wrapper).height.slice(
          0,
          -2
        );
      }
    ...
    _initialize() {
      ...
      this.canvas.style.backgroundColor =
        !this.canvasColor || this.canvasColor == "default"
          ? "transparent"
          : typeof this.canvasColor === "string"
          ? this.canvasColor
          : "";
    ...
    _setSize() {
      ...
      this.canvas.style.width =
        (this.useAutoSizing ? this.realWidth : this.width) + "px";
      this.canvas.style.height =
        (this.useAutoSizing ? this.realHeight : this.height) + "px";
    },
    ...
    _setTextPlaceholder() {
      ...
      let defaultFontSize =
        (this.outputWidth * DEFAULT_PLACEHOLDER_TAKEUP) /
        this.placeholder.length;
      let fontSize =
        !this.computedPlaceholderFontSize ||
        this.computedPlaceholderFontSize == 0
          ? defaultFontSize
          : this.computedPlaceholderFontSize;
      ctx.font = fontSize + "px sans-serif";
      ctx.fillStyle =
        !this.placeholderColor || this.placeholderColor == "default"
          ? "#606060"
    ...
    _setInitial() {
      ...
      if (this.initialImage && typeof this.initialImage === "string") {
        ...
        typeof this.initialImage === "object" &&
    ...
    _handlePointerEnd(evt) {
      ...
      if (this.pointerStartCoord) {
        ...
        pointerMoveDistance =
          Math.sqrt(
            Math.pow(pointerCoord.x - this.pointerStartCoord.x, 2) +
              Math.pow(pointerCoord.y - this.pointerStartCoord.y, 2)
          ) || 0;
      }
      if (this.disabled) return;
      if (!this.hasImage() && !this.disableClickToChoose) {
        ...
        if (
          pointerMoveDistance < CLICK_MOVE_THRESHOLD &&
          tabEnd - this.tabStart < MIN_MS_PER_CLICK &&
          this.supportTouch
        ) {
          this.chooseFile();
        }
    ...
    _handleDragEnter(evt) {
      this.emitNativeEvent(evt);
      if (this.passive) return;
      if (this.disabled || this.disableDragAndDrop || !u.eventHasFile(evt))
        return;
    ...
    _setOrientation(orientation = 6, applyMetadata) {
      ...
      if (orientation > 1 || useOriginal) {
        ...
        var _img = u.getRotatedImage(
          useOriginal ? this.originalImage : this.img,
          orientation
        );
        _img.onload = () => {
          this.img = _img;
    ...
    _clipPathFactory(x, y, width, height) {
      let ctx = this.ctx;
      let radius =
        typeof this.imageBorderRadius === "number"
          ? this.imageBorderRadius
          : !isNaN(Number(this.imageBorderRadius))
